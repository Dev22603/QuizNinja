...
class DocScanner(object):
    ...
    def scan(self, image_path, output_path):
        ...
        cv2.imwrite(output_path + '/' + basename, thresh)
        print("Proccessed " + basename)


if __name__ == "__main__":
    # Set parameters directly in code
    interactive_mode = False  # Change to True if manual corner selection is needed
    image_path = None  # Set to a single image path if needed
    image_directory = "sample_images"  # Set to a directory containing images
    ...
    # Scan all valid images in directory specified by command line argument --images <IMAGE_DIR>
        ...
        im_files = [f for f in os.listdir(
            image_directory) if get_ext(f) in valid_formats]
        for im in im_files:
            scanner.scan(image_directory + '/' + im)
